...
\title{Applying Suppor Vector Machine on Breast Cancer Wisconsin dataset}
...
% in the abstract
\begin{abstract}
In machine learning, an SVM is a classifier based on the idea that the best seperating hypeplane to seperate the  datapoints belonging to different classes is the hyperplane with the maximum margin and least trainng error.We study the performance of the SVM for the classification of the Breast Cancer Wisconsin Dataset.The system developed performs with 94.63\% accuracy for the  test sample in a two class classification\\
...
In case of Non linear classifier we use kernel trick to go into higher dimensions and then make the data linearly classifiable. 
...
where  $ \mathbf{w}$ denotes the vector of weights,  $ \mathbf{x}$ is the vector of inputs, $ C$ is the cost of error making 
  ...
  .
\section{Experiments}
	The training data of this exercise is formed by a set of Breast Cancer Wisconsin Dataset and each instance belong to one of two different classes; one of the classes is Malignant and the other is Benign.We will use SVM classifier from scikit learn to classify the data int these classes.Now we will set up SVM’s parameters.We have to definr the kernel, cost of error(C) and give the training data.For non linear classication we can use different kernels such as Gausian radial basis function('rbf').In such case we have to give a parameter known as gamma to the SVM classifer.Gamma is the RBF kernel.After this we will fit our data in the SVM classifier.Now we will use the test data to test the model's accuracy using score method.
\begin{figure}[H]
\centering
\includegraphics[width=3.5in,height=2.5in]{8_2.png}
% where an .eps filename suffix will be assumed under latex, 
% and a .pdf suffix will be assumed for pdflatex; or what has been declared
% via \DeclareGraphicsExtensions.
\caption{}
...
\section{Conclusion}
After the classification we can see that the classification was  very effective,.This indeed suggest that the SVM model is ver apt to classifify the data into two class Benign and Malignant for the breast cancer dataset
The system developed performs with 94.63\% accuracy for the  test sample in a two class classification.Also we used Pipeling to check on different parameters value.We got a best result of 94.6\% for liner kernel and value of C = 10.0\\
